...
export default function ScreenStakes() {
  ...
  // Chart.js loader + chart lifecycle
  useEffect(() => {
    ...
    const renderChart = () => {
      ...
      const actuals = [2.92, 2.50, 3.37, 2.17, 1.83, 2.92, 2.80, 2.23];
    ...
    >
      <div
        style={{
          ...
          color: "var(--ss-text)",
          boxShadow:
            "0 0 0 11px #1a1a1a, 0 0 0 12px #2a2a2a, 0 30px 80px rgba(0,0,0,0.45), 0 10px 30px rgba(0,0,0,0.25)",
          fontFamily: "'DM Sans', sans-serif",
        }}
      >
        {/* Notch */}
...
/* ---------- Onboarding ---------- */
function Onboarding({ slide, setSlide, onDone, dark }) {
  const slides = [
    {
...
function OnbVisual1() {
  // Stake going into a pot — small coins flowing into a vessel shape
  return (
...
function OnbVisual2() {
  // Two horizontal lines — baseline above, goal below — with a band between
  return (
    <svg viewBox="0 0 260 200" width="260" height="200">
      ...
      <line
        ...
        strokeDasharray="4 4"
      />
      <text x="244" y="63" fontFamily="DM Sans" fontSize="10" fill="var(--ss-text2)">
        baseline
      </text>
      <text x="0" y="63" fontFamily="DM Sans" fontSize="10" fill="var(--ss-text2)">
        5h 20m
      </text>
      {/* goal */}
      <line
        ...
        strokeDasharray="4 4"
      />
      <text x="244" y="133" fontFamily="DM Sans" fontSize="10" fill="var(--ss-gold)">
        goal
      </text>
      <text x="10" y="133" fontFamily="DM Sans" fontSize="10" fill="var(--ss-gold)">
        3h
      </text>
      {/* band between */}
      <rect x="30" y="60" width="210" height="70" fill="var(--ss-accent)" opacity="0.08" />
      {/* curve dipping below goal */}
      <path
        d="M 40 80 Q 80 100 110 110 T 180 145 T 230 140"
        stroke="var(--ss-accent)"
        strokeWidth="2"
        fill="none"
      />
...
function OnbVisual3() {
  // Time large above, money small below — the spec says "show through the visual layout"
  return (
...
function ChallengeTab({ dark, setDark, onInvite, stake, setStake, goalHours, setGoalHours }) {
  // D8 — under goal day. 2h 14m used. 3h 6m reclaimed today.
  const days = [
    { d: 1, h: 2.92, win: true },
    { d: 2, h: 2.5, win: true },
...
function StatsTab({ chartCanvasRef }) {
  const days = [
    ...
    { d: 8, h: 2.23, win: true, pot: null }, // in progress
  ];
  const baseline = 5.33;
  ...
  return (
    <div className="ss-fade">
      ...
      {/* Money summary — smaller than time */}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: "12px 0",
                  borderBottom:
                    i < days.length - 1 ? "1px solid var(--ss-border)" : "none",
                }}
              >
                <div
                  style={{
          ...
          {menu.map((m, i) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                borderBottom:
                  i < menu.length ? "1px solid var(--ss-border)" : "none",
                textAlign: "left",
              }}
            >
...
function MenuIcon({ name }) {
  ...
  const sw = 1.4;
  const common = { width: 18, height: 18, fill: "none", stroke, strokeWidth: sw };
  switch (name) {
    case "card":
      return (
...
function NavIcon({ name, active }) {
  ...
  const common = { width: 22, height: 22, fill: "none", stroke, strokeWidth: sw };
  switch (name) {
    case "target":
      return (
        <svg viewBox="0 0 24 24" {...common}>
